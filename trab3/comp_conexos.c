...
int **create_image(width, height) int width, height;
{
  int i;
  int **prow, *pdata;
...
void equiv_iterative(p, q) int p, q;
{
  int temp;
...
void equiv_recursive(p, q) int p, q;
{
  int temp;
  ...
  for (row = 0; row < height; row++)
    for (col = 0; col < width; col++)
    {
      ...
      if (k != -1)
      {
        ...
        binary_image[row][col] = BACKGROUND;
    }
  for (k = 0; k < num_comp; k++) {
...
void read_image_pbm(argc, argv) int argc;
char **argv;
{
  unsigned char ch;
  ...
  for (row = 0; row < height; row++)
    for (col = 0; col < width_bytes; col++)
    {
      ...
      for (bits = 0; bits < 8 && col * 8 + bits < width; bits++)
        if (ch & mask[bits])
          binary_image[row][col * 8 + bits] = FOREGROUND;
        else
          binary_image[row][col * 8 + bits] = BACKGROUND;
    }
...
void save_image_pbm(argc, argv) int argc;
char **argv;
{
  unsigned char ch;
  int width_bytes, bits;
